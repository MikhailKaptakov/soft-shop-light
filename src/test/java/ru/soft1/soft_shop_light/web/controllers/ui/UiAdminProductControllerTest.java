...
class UiAdminProductControllerTest extends AbstractControllerTest {
    ...
    *   для алфавитного указателя сделать на главной странице гармошку (как шторку) раскрывающую
    * область с навигационной панелью (кнопкам для переключения сортировки - по названию, по вендору, по тсране производителю)
    * после нажатия кнопки поиска, генерируется алфавитный указатель и кликабельный список
    * (при нажатии на элемент списка разворачивается модальное окно как при нажатии на кнопку подробнее
    *  (при этом делается запрос бд по конкретной сущности))
    * (Делает запрос данных из бд)
    * При запросе к бд - вытаскиваем по getAllAvailable();
    * */

    /* TODO Окно поиска -
    *   Разворачиваемая панель поиска в которой окошко поиска и радио-элементы для переключения типа поиска (по вендору и прочее)
    *   Данный поиск делает запрос к базе данных и сменяет отображаемые карточки товаров.
    *  */
    ...
    /* TODO страницы товара -
     *   Добавить кнопку для открытия товара в отдельной странице
